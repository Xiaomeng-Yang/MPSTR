...
class MPNet(BaseSystem):
    ...
    def iterative_decode(self, feature_map: torch.Tensor, target: Optional[Tensor] = None,
        ...
        token_logits = torch.zeros([bs, self.max_label_length + 1, len(self.tokenizer) - 3], dtype=torch.float, device=self._device)
    ...
    def forward_logits_loss(self, images: Tensor, labels: List[str]) -> Tuple[Tensor, Tensor, int]:
        ...
        loss = F.cross_entropy(logits.flatten(end_dim=1), tgt_out[:, :logits.shape[1]].flatten(), ignore_index=self.pad_id)
